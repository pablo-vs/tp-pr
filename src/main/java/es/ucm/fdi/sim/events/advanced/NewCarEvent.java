...
public class NewCarEvent extends NewVehicleEvent {
	 ...
	 *            Seed for the PRNG.
	 */
	public NewCarEvent(int t, int v, String id, List<String> it, int resist,
			double prob, int faultMax, long seed) {
		super(t, v, id, it);
		resistance = resist;
		faultProb = prob;
	 ...
	 *            Seed for the PRNG.
	 */
	public NewCarEvent(NewVehicleEvent e, int resist, double prob,
	...
	public Car createCar(RoadMap r) {
		Car result;
		try {
			result = new Car(vehicleID, maxSpeed, createItinerary(r),
					resistance, faultMax, faultProb, seed);
	...
	public static class Builder extends NewVehicleEvent.Builder {
		 ...
		 */
		@Override
		public NewCarEvent build(IniSection ini)
